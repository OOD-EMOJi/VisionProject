...
public class CoinBotBehavior implements RobotBehavior{
    ...
    public Command getCommand(Location location){
		if(!location.getCoins().isEmpty()) return new CommandCoin(robot); 
        // Make paths
		int x = location.getX();
        int y = location.getY();
    ...
    public DirType getDirection(int x1, int y1, int x2, int y2){
            ...
            if (x2-x1==0 && y2-y1==1)  return DirType.East;
            if (x2-x1==1 && y2-y1==0)  return DirType.South;
            if (x2-x1==0 && y2-y1==-1)  return DirType.West;
            if (x2-x1==-1 && y2-y1==0)  return DirType.North;
			else return null;
    }
}
